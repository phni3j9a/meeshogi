#!/usr/bin/env bash
# Resume the Android acceptance suite from analysis-partial-review onward on the
# current install (assumes android-acceptance.sh already ran licenses through
# analysis-review and pushed the kiou fixtures). Same recording/logcat harness.
set -euo pipefail
...
record_loop() {
  ...
  while :; do
    [[ -e "$record_stop" ]] && break
    segment=$((segment + 1))
    local remote="/sdcard/meeshogi-acceptance-${segment}.mp4"
...
echo "Acceptance resume run finished: $run_dir"
