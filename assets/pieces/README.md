...
先手は上記の生成画像を使う。後手はChatGPT Imageで個別に編集生成した[gote/の専用14画像](gote/prompts.json)を使う。元の字形・輪郭を参照し、木地を明るい艶のある飴色へ変更した。中間色に濃さと彩度を持たせ、明るい部分と墨色／朱色の文字を残す。全体への色重ねは撤去し、`PieceImage`は現在の持ち主に合う画像1枚を描く。盤反転では色を入れ替えず、捕獲・駒打ちに追従する。

後手用も透明余白のトリミング、224×256px以内への縮小、8pxの透明余白追加、PNGパレット最適化のみを行う。生成後の色補正・文字の描き足しは行わない。元の15画像は保持し、現在のアプリには先手14画像と後手14画像を静的に同梱する。元の`jewel.png`は生成履歴として残るが、後手の玉には`gote/jewel.png`を使う。
