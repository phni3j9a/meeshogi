...
先手は生成画像そのままの暖かい木色、後手は黄色みを少し抑えた木色で表示する。後手だけ同じ画像を`#526879`・不透明度`0.14`で薄く重ね、画像の透過形状と木目・墨色／朱色の文字を保つ。これは`PieceImage`の表示スタイルであり、PNGや生成プロンプトは変更しない。先後の色は現在の持ち主に従い、盤反転では入れ替わらず、捕獲・駒打ちに追従する。
