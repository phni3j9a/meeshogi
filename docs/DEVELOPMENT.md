...
先後の駒の色味にも小さな差を付けた。追加の要望を反映し、先手は生成画像の暖かい木色、後手は暗い茶色を重ねた少し濃い飴色に調整した。共通の`PieceImage`に適用し、盤上・持駒・詰み手順で揃えた。色は現在の持ち主に従い、盤反転で入れ替わらない。PNG・生成プロンプト・依存ライブラリの変更はない。色味変更のWeb操作結果は[専用ログ](design/screens/analysis-refresh/piece-tones-amber-verification.json)を参照する。

型検査と63テストが通過した。ExpoのiOS・Android向けHermesバンドル生成も成功し、両方へ全15画像が同梱されることを確認した。Expo依存チェックはオフラインモードで一致したが、オンラインの確認はプロキシ接続タイムアウトのため完了していない。依存の追加・更新はない。
