...
先後の駒の色味にも小さな差を付けた。先手は生成画像の暖かい木色、後手は同じ画像への薄い色重ねで落ち着いた木色にする。共通の`PieceImage`に適用し、盤上・持駒・詰み手順で揃えた。色は現在の持ち主に従い、盤反転で入れ替わらない。PNG・生成プロンプト・依存ライブラリの変更はない。色味変更のWeb操作結果は[専用ログ](design/screens/analysis-refresh/piece-tones-verification.json)を参照する。
