...
iOSは[通常CI 34695873539](https://github.com/phni3j9a/meeshogi/actions/runs/34695873539)でRelease Simulatorアプリをビルドし、製品ソース・来歴・SHA-256の一致を確認した[再検証 34704143419](https://github.com/phni3j9a/meeshogi/actions/runs/34704143419)で、取り込み・保存・実解析・再起動とページ撮影が全て通過した。主要9画面と対局情報・ライセンスの21枚を目視し、名前の保存と戦績反映、戦型の手動修正と詳細、67手目の3手詰め回答、候補分岐まで確認した。ライセンス画面の目視で、戻るタイトルに内部ルート名が出る点とパッケージ一覧が原文より目立つ点を発見し、「設定」への戻る表示、原文を先に読める配置と全パッケージの改行一覧へ修正した。この表示変更を含む両OSのビルドと画面確認を進めている。

iOSの撮影では、一覧表示は初回・再起動とも180秒、棋譜読み取りは60秒、保存後の画面遷移は120秒まで待機する。Simulatorで保存や起動が従来の待機上限を超えた記録があり、応答時間を実機性能の検証済み値とはしない。

Android CIのAPI 36では、[CI 34695873539](https://github.com/phni3j9a/meeshogi/actions/runs/34695873539)で取り込み・名前設定・全局解析・分岐・ファイル取り込み・戦績修正・ライト／ダーク・KIF出力・背景停止・文字拡大・検索削除の全13フローが通過した。共有シートを展開して回収したKIFは原本とバイト単位で一致し、両方3,174 bytesだった。対象画面を目視し、クラッシュ・ANR・メモリ不足・React Nativeの例外は記録されていなかった。ローカルのAPI 36でも、保存処理の更新後に同じ現行モデルで81局面の解析完了を確認した。ライセンス表示の仕上げを含む最終確認が残っているため、無料版全体の受入完了とはしていない。2026年9月12日のユーザー指示により、今回のiOSの完了条件はSimulatorで各ページのスクリーンショットを撮り、実際に目視することとした。詳細な操作の自動検証、文字拡大・全テーマ・実機性能は残事項として区別する。Androidの操作検証と共通ロジックの確認は従来どおり継続する。
