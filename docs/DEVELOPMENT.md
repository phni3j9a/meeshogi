...
ChatGPT Imageで15種類の駒を個別生成し、盤上・持駒・詰み手順で共有する画像表示へ変更した。盤面の幅、直前手の強調、対局者と持駒の配置、評価値カード、候補手の読み筋、固定手送りを整理した。詳細と生成プロンプトは [デザイン記録](design/analysis-refresh.md) を参照する。
...
実際の画面コンポーネントをReact Native Webで表示し、320px・390px・768px幅、ライト／ダーク、手送り・盤反転・グラフ移動・候補からの分岐・本譜復帰を確認した。プレビューの棋譜は既存KIF、解析結果は表示検証用の合成値であり、ネイティブ解析の実行証拠ではない。
