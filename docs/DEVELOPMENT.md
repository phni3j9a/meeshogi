...
先後の色味は、全体に色を重ねる方法がくすんで見えるという指摘を受け、後手用14画像をChatGPT Imageで編集生成する方式へ変更した。木地の中間色に濃さと彩度を持たせ、明るい部分と墨色／朱色を保つ。`PieceImage`の色重ねを撤去し、持ち主に合う画像1枚を描く。盤上・持駒・詰み手順で共有し、反転・捕獲・駒打ちに追従する。元の15画像と生成プロンプトは保持し、後手用の生成記録を追加した。専用の[Web検証ログ](design/screens/analysis-refresh/piece-generated-gote-verification.json)を参照する。

型検査と63テストが通過した。ExpoのiOS・Android向けHermesバンドル生成も成功し、両方へ先後それぞれ14画像が同梱されることを確認した。Expo依存チェックはオフラインモードで一致したが、オンラインの確認はプロキシ接続タイムアウトのため完了していない。依存の追加・更新はない。
