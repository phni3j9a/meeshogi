...
The deployed staging image runs contract v3: the fixed-SFEN gate, single-legal-move and `middlegame-150` probes, and the selected-profile re-runs on the original fixture set all pass — see [Post-fix verification](#post-fix-verification-2026-09-23-image-sha25684599cdf-contract-v3). This checkout contains a W3 async-job backend repair and additive migration `0002`; they have **not** been applied to staging or deployed. The live W3 record below describes the earlier backend build only and is not acceptance evidence for this repair. Production resources remain absent; on-device analysis remains in the app during evaluation.
