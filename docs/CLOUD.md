...
Admission order is: six new-key POSTs/minute per principal (same-key replays exempt), kill/profile-block/daily-cost-cap, at most ten queued/running/cancelling jobs globally, one active job per owner, then UTC-day owner quota. Free quota is 5 jobs / 1,024 positions; precision is 2 jobs / 512 positions; each job is limited to 512 analyzed positions (initial state plus up to 511 moves). GET/profile/results share 120 operations/minute per principal; cancel allows 30/minute. Every 429 carries `Retry-After`. Rejected admissions do not reserve daily quota. Owner quota usage remains consumed once admitted. Outstanding reservations from all UTC days count against today's spend cap. Daily cost reservation is separate: each admission reserves a conservative container-resource estimate for all positions, two bounded attempts, proof, readiness/cold start, and idle time. Completed work and unused reservation are separated; attempt settlement is idempotent, including cancel, timeout, and confirmed destruction after unknown transport. Cache hits have zero engine-attempt cost but still consume their share of container reservation. The job API exposes admission estimate and settled attempt estimate separately; observed engine invoice cost is unavailable. Workers/DO/D1/Queue charges are listed separately and excluded from the bound. Warning begins at $0.50, cap at $1.00 per UTC day. These are estimates, not an invoice.

Admission atomically stores the job, positions, owner quota, daily cost reservation, idempotency key, and size-8 outbox rows. It sends only the head unfinished chunk. Claim and global slot acquisition share one D1 batch; a failed step rolls back both. A fenced commit advances a job-monotonic `result_seq`, completes its outbox row when terminal, and sends the next chunk. Queue sends increment `delivery_count`; engine dispatch increments `attempts` (maximum 2). The jobs queue uses `max_retries: 5` so a chunk can reach the job-level failure threshold before its message enters the DLQ. Queue order is not relied on. Results are paged by `result_seq`, then displayed by `position_index`; every page returns `resumeCursor`, including an empty tail page (which echoes the supplied cursor). A one-minute `scheduled()` scan resends stale outbox heads, resumes both explicit-cancel and DLQ intermediate states, recovers expired leases only after `Container.destroy()` confirms process death, terminates jobs that exceed the no-progress deadline, and removes terminal jobs/results after 7 days and cache rows (including quarantine) after 30 days. Quota, attempt-cost, and daily aggregates are retained.
...
- **Configurable cost cap** (`af0d453`): `ANALYSIS_DAILY_COST_CAP_USD` (default `1`, staging `5`). The conservative reservation model prices a 78-position job at ≈$2.43 free / ≈$4.16 precision, so a hardcoded $1 cap made full-game admission impossible; admission, position acquisition, and cost snapshots now share the configured cap (warning at half).
