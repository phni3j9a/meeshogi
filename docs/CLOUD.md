...
The deployed staging image runs contract v3: the fixed-SFEN gate, single-legal-move and `middlegame-150` probes, and the selected-profile re-runs on the original fixture set all pass — see [Post-fix verification](#post-fix-verification-2026-09-23-image-sha25684599cdf-contract-v3). The W3-fix backend (dual container classes, migration `0002`, hardened coordinator) is applied and live-verified on staging — see [Post-repair live W3 verification](#post-repair-live-w3-verification-2026-09-24-version-71a8d636--image-sha256b0818e4f). Production resources remain absent; on-device analysis remains in the app during evaluation.
...
Owner-authenticated routes are `POST /v1/jobs`, `GET /v1/analysis-profiles`, `GET /v1/jobs/{id}`, `GET /v1/jobs/{id}/results?cursor=&limit=`, and `POST /v1/jobs/{id}/cancel`. `POST /v1/jobs` accepts `{ idempotency_key, profile, initialSfen, moves, label? }`: `initialSfen` is index 0, then every successfully replayed USI move adds the resulting position. `moves: []` creates a single-position job. `tsshogi@2.3.4` validates/replays at most 512 moves; an illegal move returns HTTP 400 and its zero-based `moveIndex`. The idempotency digest covers normalized `{ initialSfen, moves, profile }`; `label` is metadata. The old public `positions[]` input is rejected. New jobs return `202`; same-owner/key/payload replay returns `200`; same key with another payload returns `409`.

Profiles are server-fixed, both at version 2: `free-v1` uses `standard-2` (1 vCPU), 1,000 ms / MultiPV 2 / Threads 1 / Hash 256 MiB; `precision-v1` uses `standard-3` (2 vCPU), 2,000 ms / MultiPV 3 / Threads 2 / Hash 256 MiB. `GET /v1/analysis-profiles` returns each profile's public execution-identity components and hash, entitlement for the caller, and blocked state. Job and result envelopes carry the same `executionIdentityHash`; contract-v3 result bodies remain unchanged. The hash includes CPU type/count, all effective search options, engine binary digest label, engine/model IDs, parser, helper, proof, contract, and TT-reset/history revisions. Cache keys include the identity hash; migration `0002` retains legacy cache rows but marks them quarantined and unreadable.

Admission order is: six new-key POSTs/minute per principal (same-key replays exempt), kill/profile-block/daily-cost-cap, at most ten queued/running/cancelling jobs globally, one active job per owner, then UTC-day owner quota. Free quota is 5 jobs / 1,024 positions; precision is 2 jobs / 512 positions; each job is limited to 513 analyzed positions (initial state plus up to 512 moves). GET/profile/results share 120 operations/minute per principal; cancel allows 30/minute. Every 429 carries `Retry-After`. Rejected admissions do not reserve daily quota. Owner quota usage remains consumed once admitted. Daily cost reservation is separate: each admission reserves a conservative estimate for all positions plus both container idle windows; completed positions and undispatched positions on cancel/failure release reservation, while each engine attempt is recorded once in an immutable attempt ledger. Cache hits have zero engine-attempt cost but still consume their share of container reservation. Warning begins at $0.50, cap at $1.00 per UTC day. These are estimates, not an invoice.

Admission atomically stores the job, positions, owner quota, daily cost reservation, idempotency key, and size-8 outbox rows. It sends only the head unfinished chunk. A fenced commit advances a job-monotonic `result_seq`, completes its outbox row when terminal, and sends the next chunk. Queue sends increment `delivery_count`; engine dispatch increments `attempts` (maximum 2). Queue order is not relied on. Results are paged by `result_seq`, then displayed by `position_index`; every page returns `resumeCursor`, including an empty tail page (which echoes the supplied cursor). A one-minute `scheduled()` scan resends stale outbox heads, recovers expired leases only after `Container.destroy()` confirms process death, terminates jobs that exceed the no-progress deadline, and removes terminal jobs/results after 7 days and cache rows (including quarantine) after 30 days. Quota, attempt-cost, and daily aggregates are retained.

Only contract-v3 `ok` / `mate` are evaluation successes and cache eligible. `incomplete` becomes a failed position with detail `incomplete`, no retry and no cache. `cancelled`, `position_failed:*`, protocol errors, and unknown terminals never become successes; timeout/engine-exit/restart receive at most one fresh engine attempt. Illegal PV, illegal bestmove, unknown score, and other protocol/integrity errors fail the position, stop its job, and block that profile until an admin clears `/v1/internal/profiles/{profile}/clear-block`. Verified `no_legal_moves`, `none`, and `win` are processed terminal states, not evaluations and not cached; `resign` is reported as evaluation-missing. Job counts separate `succeeded`, `incompleteOrMissing`, `failed`, and `terminal`. Mate-result rows call the bounded `/prove` driver endpoint (3 plies, 10,000 helper budget); the envelope stores proof revision/result/cost or `null`. A proof result never changes the strict analysis body or creates a badge from an engine score.

Cancellation writes `cancelling` before it contacts the exact profile container's `/stop` endpoint. It blocks new dispatch and fences results; if the process has not quiesced after 5 seconds, the worker calls `destroy()` and only settles `cancelled` after destruction is confirmed. Cancellation before start completes immediately; terminal jobs keep their existing status. Stale/unknown DLQ messages are acknowledged. A current-epoch DLQ message destroys any active lease, preserves committed results and consumed cost, releases outstanding cost reservation and terminates the job as `partial` or `failed`; it is never sent back to the source Queue.

`/v1/jobs/{id}` returns identity, counts, timestamps, stop reason and cost. Results contain committed `done` and terminal `failed` rows with identity, result, stats, proof and separate delivery/attempt counts. Auth uses SHA-256 token digests in D1 `principals`; cross-owner IDs return 404. `ANALYSIS_ADMIN_TOKEN` protects internal kill, fault-arm, and profile-block clear routes. Fault fixtures are armed via `POST /v1/internal/fault/arm` only when `ANALYSIS_FAULT_FIXTURES_ENABLED=1` — enabled on the staging deployment for live fault-injection verification: admin may arm one `destroy` for a job or `throw` before claim for 1–4 deliveries (to drive the configured retry limit into the DLQ). The driver-side `MEESHOGI_TEST_SIGSTOP_ENGINE=1` is a one-shot first-search hook; its normal deadline, stop, reap, and restart code must recover the stopped child. It is unset in the staging deployment because container runtime env vars are not configurable through the Wrangler containers config; the no-response path is covered by driver unit tests and the live `destroy` arm instead.

Migration `0001_init.sql` creates the original tables. Additive `0002_async_jobs_hardening.sql` preserves admitted jobs, positions, owner quota, historical cost, and cached values, extends job state with `cancelling`, adds execution identity / result sequence / delivery / proof / reservation columns and new cost, slot, and fault tables, and marks old cache rows quarantined. Existing terminal jobs remain history. Because queued/running rows have no v2 identity, the migration closes them as `failed` or `partial` with `pre_identity_migration` and terminalizes their unfinished positions so an old Queue message cannot resume them under a new engine identity. `0002` was applied to the staging database with no active jobs at apply time (9 legacy jobs, principals, and 26 quarantined legacy cache rows preserved), and the dual-container classes, v3 Durable Object migration, scheduled handler, and DLQ consumer are live-verified on staging (see the staging verification record below).

Main applies `0001` then `0002` to the existing staging database before a staging deployment, for example:
...
Migration `0002_async_jobs_hardening.sql` applied remotely to the existing staging D1 with zero active jobs at apply time; all 9 legacy jobs, both principals, and 26 legacy cache rows were preserved — the legacy cache rows sit under `quarantined=1` and are never read. Worker version `1fb61ef2` then `71a8d636` deployed with the v3 DO migration (`AnalysisContainerPrecision`), cron `* * * * *`, and the DLQ consumer. Fresh principal `staging-w3fix-verify`/`staging-w3fix-verify2` seeded (token SHA-256 digests only). Observed on the deployed Worker:
