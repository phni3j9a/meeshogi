...
- `AnalysisContainer` is a SQLite-backed Container Durable Object with one instance maximum and a 30-second idle sleep target. The staging configs select either `standard-2` or `standard-3`; only one config is active for the shared staging Worker at a time. `JobCoordinator` is a second SQLite-backed Durable Object for global admission, rate windows, the kill flag, and cost snapshots. The config has one worker named `meeshogi-analysis-staging` and no production or named environment.
...
The Worker now exposes owner-authenticated routes under `/v1/jobs`:

- `POST /v1/jobs` accepts `{ idempotency_key, profile, positions, label? }`; each request has 1–512 strict SFEN values. Raw engine controls are rejected. A new job returns `202`; a same-owner, same-key, same-payload replay returns the original job with `200`; reusing that key for another payload returns `409`.
- `GET /v1/jobs/{id}` returns job state, pending/running/done/failed counts, profile and engine/model identity, timestamps, committed count, stop reason, this job's estimated container cost, and the global `costWarning` flag.
- `GET /v1/jobs/{id}/results?cursor=&limit=` returns committed `done` and terminal `failed` positions in stable position-index order. `limit` is 1–100. Successful rows store the shared contract-v3 result object (including `engineBestmove` when required) and separate statistics; failed positions return an error detail and no invented analysis result.
- `POST /v1/jobs/{id}/cancel` is the only client cancellation path. Cancellation is honored between positions; an already in-flight position can commit, existing results stay available, and unstarted positions remain pending. A cancelled job has status `cancelled` whether or not it has committed results.

Bearer credentials are matched against SHA-256 digests in the D1 `principals` table. A revoked or unknown token gets `401`; a free-only principal asking for `precision-v1` gets `403`; another owner's job is indistinguishable from a missing ID and gets `404`. Main seeds principal rows after deployment; no bearer values are in migrations or source. The existing `ANALYSIS_ADMIN_TOKEN` continues to protect `/v1/internal/*`; it is separate from principal credentials.

Profiles are server-fixed: `free-v1` is 1,000 ms / MultiPV 2 / Threads 1 / Hash 256 MiB, and `precision-v1` is 2,000 ms / MultiPV 3 / Threads 2 / Hash 256 MiB. Every job and result also records the profile version, engine ID, and opaque model ID. Container instance type is selected by the active staging config, not dynamically per job; both profiles use that one selected instance size during a deployment.

Admission reserves quota by owner and UTC day, including cancelled or failed jobs: free allows 5 jobs and 1,024 positions; precision allows 2 jobs and 512 positions. Each job is limited to 512 positions. The `JobCoordinator` serializes the global active-job slot (one queued/running job), six new job POSTs per minute, ten non-POST job operations per owner per minute, the kill mode, and the cost-cap check. The database batch atomically inserts the job, positions, quota reservation, idempotency row, and outbox chunks. The DO sends the chunk-size-8 outbox messages to the existing `meeshogi-analysis-staging-jobs` Queue and then marks them sent. Retrying the same idempotency key flushes an unsent outbox after a send error; a crash between Queue send and outbox mark can duplicate a message safely.

The Queue consumer runs one message at a time, processes positions in index order, and commits each result before acknowledging the message. Position attempts increase only at engine dispatch. A lease makes a pending/running position resumable after a worker failure; engine timeout, engine exit, or protocol failure gets one fresh dispatch retry, then a terminal failed row. Three consecutive failures or five total failed positions stops the job: it becomes `failed` with no committed result or `partial` with at least one. Normal completion is `completed`. Queue deliveries are at-least-once, and a duplicate delivery skips already-terminal rows and cannot add a second result or cost row. Queue transient failures are retried three times and then moved to the configured DLQ. There is no DLQ redrive endpoint yet; a chunk that exhausts retries can leave its job active with pending work until Main provides a redrive procedure.

The result cache matches only contract version 3, engine ID, model ID, profile ID/version, and the exact SFEN. A hit copies the complete validated result into the position and records zero engine/container cost; job status includes the cached-position count and estimated avoided container cost. Successful and terminal failed positions have one idempotent cost-ledger row. The estimate uses at least the server movetime plus request overhead, prices failed dispatches at the driver's movetime-plus-five-second deadline, and adds one 30-second container idle window per position. This deliberately overestimates a warm job; it excludes Workers, DO, Queue, D1, egress, logging, and account allowances and is not an invoice. Responses include `costWarning: true` from $0.50 global accumulated estimate. At $1.00 the Worker refuses admission with `503 cost_cap` and stops dispatching further positions, ending affected jobs as `partial` or `failed`. Admin can set `/v1/internal/kill` to `admission` or `all` and clear it with `DELETE`.

`cloud/migrations/0001_init.sql` creates principals, jobs, positions, idempotency, daily quota, result cache, cost ledger, flags, and the recoverable queue outbox. Main applies it to the existing staging database before deployment, for example:
