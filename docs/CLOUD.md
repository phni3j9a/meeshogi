...
Staging Worker `c041494a` remains deployed with code `7a0c93a` on image `sha256:3908907a…`; remote D1 migrations `0001`–`0005` are applied. The 104-position precision job (`98401ddc`, public Asahi Cup final game) demonstrated rolling chunk reservations and phase settlement under the `$1` daily cap on the earlier deployment (`0f0c33b6`); see the W3F-02 live-verification section. The remaining R3/R4/R5 fixes described below are local changes and have not been rebuilt, deployed, or live-verified. Migration `0006_driver_lifetime_restart_identity.sql` is also local and still needs to be applied before a deployment that includes it. The deployed backend already routes Queue, cron, and same-key replay through a shared chunk transition and terminal cost finalizer, checks required settlements before releasing reservations, preserves lifecycle events locally when D1 is unavailable, and records budget overruns without clipping observations. Production resources remain absent; on-device analysis remains in the app during evaluation.
