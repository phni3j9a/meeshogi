...
Deployed staging Worker `0f0c33b6` runs the pre-repair Cost envelope v2 code (`c1adcf9`) on the pinned image (`sha256:3908907a…`). Migration `0004_chunk_cost_ledger.sql` is applied remotely. A 104-position precision job (`98401ddc`, public Asahi Cup final game) demonstrated rolling chunk reservations and phase settlement under the `$1` daily cap; see the W3F-02 live-verification section. Independent review reproduced five defects (R1–R5). The source now routes Queue, cron, and same-key replay through a shared chunk transition and terminal cost finalizer, checks required settlements before releasing reservations, preserves lifecycle events locally when D1 is unavailable, and records budget overruns without clipping observations. These source repairs have not been deployed or verified on staging; migration `0005_cost_finalization_budgets.sql` is pending remotely. Production resources remain absent; on-device analysis remains in the app during evaluation.
