...
Staging Worker version `1c06a489` (code `e04a07c`) is deployed on image `sha256:410b92aa…`; remote D1 migrations `0001`–`0006` are applied. This deployment completes W3-fix5: driver-lifetime (`driverEpoch`) restart deduplication, confirmed container stop before `sleep_confirmed`, bounded lifecycle RPC flushing, explicit zero-cost settlement for untouched next positions, finite persisted budget intents, and unclipped observed durations. The 104-position precision job (`98401ddc`, public Asahi Cup final game) demonstrated rolling chunk reservations and phase settlement under the `$1` daily cap on the earlier deployment (`0f0c33b6`); see the W3F-02 live-verification section. The deployed backend routes Queue, cron, and same-key replay through a shared chunk transition and terminal cost finalizer, checks required settlements before releasing reservations, preserves lifecycle events locally when D1 is unavailable, and records budget overruns without clipping observations. Fault fixtures remain disabled (`ANALYSIS_FAULT_FIXTURES_ENABLED=0`). Production resources remain absent; on-device analysis remains in the app during evaluation.
...
- The driver reports a stable `driverEpoch` for one Python controller lifetime and a changing `engineEpoch`, `processId`, and cumulative `restartCount` for engine processes in health and analysis responses. Restart events are deduplicated by `(profile, driverEpoch, restartCount)`, so observing one restart first in an analysis result and again in health does not spend the runtime budget twice; a new controller lifetime starts a new sequence. Search deadline is `movetime + 5,000 ms`; timeout sends `stop`, waits at most one second, reaps via SIGTERM then SIGKILL if needed, and starts a fresh, re-verified process. Restart and startup readiness attempts are bounded. Timeout returns `position_failed:engine_timeout`; `incomplete` means the search completed without a full initial iteration and is not a retry hint.
...
Health reports the USI engine identity, short binary, weight, and options SHA-256 prefixes, `driverEpoch`, `engineEpoch`, `processId`, `restartCount`, `lastRestartReason`, CPU flags (including whether `avx2` is present), readiness, and best-effort process/cgroup statistics. Analysis responses carry the same `driverEpoch` and the process epoch/PID that produced the result, even when a timeout then starts another process. New Worker results require `driverEpoch`; the shared v3 validator keeps it optional for older stored result rows. Benchmark analysis also includes process peak/current RSS, cumulative engine CPU time, and cgroup memory when readable. Missing statistics are omitted and never fail a request.
...
Profiles are server-fixed, both at version 2: `free-v1` uses `standard-2` (1 vCPU), 1,000 ms / MultiPV 2 / Threads 1 / Hash 256 MiB; `precision-v1` uses `standard-3` (2 vCPU), 2,000 ms / MultiPV 3 / Threads 2 / Hash 256 MiB. `GET /v1/analysis-profiles` returns each profile's public execution-identity components and hash, entitlement for the caller, and blocked state. Job and result envelopes carry the same `executionIdentityHash`; new contract-v3 result bodies include `driverEpoch` as an additive field. The hash includes CPU type/count, all effective search options, full engine/weight/options/helper/driver SHA-256 digests from configuration and runtime provenance, engine/model IDs, parser, helper, proof, contract, and TT-reset/history revisions. Before dispatch, cache lookup, and commit, the Worker compares the full `/health` provenance and configured engine digest; missing or mismatched provenance fails closed. Rebuild the private image and update `ANALYSIS_WEIGHT_SHA256`, `ANALYSIS_ENGINE_OPTIONS_SHA256`, `ANALYSIS_HELPER_SHA256`, and `ANALYSIS_DRIVER_SHA256` to the artifact's values before enabling job admission; this local driver change changes its digest and therefore the execution identity. Cache keys include the identity hash; migration `0002` retains legacy cache rows but marks them quarantined and unreadable.
...
### W3-fix5 live verification (2026-09-24, version `1c06a489`, image `sha256:410b92aa…`, migrations `0001`–`0006` applied)

The W3-fix5 changes (`e04a07c`: R3 unused-inter-position settlement, R4 bounded lifecycle RPC plus confirmed-stop/destroy fallback, R5 `driverEpoch` restart dedup and unclipped budget observations) required a driver change, so the private image was rebuilt as tag `v7` without provenance attestations (the managed registry rejected the attestation manifest on the first push) and pushed as `sha256:410b92aa12489e1f58cf40ec04ff61364dca12cd9b6824be8858df623ba9ddf4`. `ANALYSIS_DRIVER_SHA256` (`54ffb370…`) and `ANALYSIS_HELPER_SHA256` (`776e6204…` — the helper binary hash changed without source changes due to Rust toolchain reproducibility; the recorded value is the deployed artifact's actual digest) were updated to match, which rotates the execution-identity hash to `8c5f87dd…` for `free-v1`. Migration `0006` was applied remotely with zero active jobs. Observed live:

- **New image on both profiles**: `/v1/internal/health` on `free-v1` and `precision-v1` returns `driverEpoch` alongside `engineEpoch`, `restartCount`, and `processId` — fields only the new driver emits. Both container applications rolled to the new image (`provisioning` → live).
- **End-to-end job on the new identity**: a 4-position crafted job (`95ec88f3`, `free-v1`, bound to the new identity hash, admitted via SQL because all principal daily quotas were exhausted) completed `4/4` done, attempts 1 each, `cost_finalized=1`.
- **Restart dedup evidence**: `cost_phase_ledger` `container_restart` rows for the job carry `driverEpoch`, `engineEpoch`, `restartCount`, `newlyObservedRestarts`, finite `budgetMs:420000`, and `observedDurationMs` — 17 observations settled at `$0` with no restarts, demonstrating the driver-lifetime dedup key.
- **Settlement and release**: all 20 reservation parts settled to `remaining_usd=0` (`currentOutstandingReservationUsd` 0); `inter_position_runtime` recorded real small amounts, including the zero-cost path for untouched next positions.
- **Stop confirmation**: `container_lifecycle_events` shows `first_contact` → `sleep_timer_elapsed` → `sleep_confirmed` with `{"reason":"exit","exitCode":0}` — `sleep_confirmed` is written only after the container's actual exit was observed (R4).
- **Fault fixtures**: `ANALYSIS_FAULT_FIXTURES_ENABLED=0` on the deployed config; arm routes return `not_found`.
...
Synthetic tests and helper tests do not independently verify the live deployment or invoice. The earlier staging smoke, AVX2 response, and post-fix live verification (W3-fix2, W3F-02, W3-fix5 sections above) are recorded; the current image `sha256:410b92aa…` is deployed and live-verified. Old benchmark data remains available but is not post-fix acceptance evidence.
