...
2026-09-09に、UI具体化のための [HTMLモック](mock/README.md) を追加した。外部ライブラリなしのHTML/CSS/JavaScriptと固定の局面データで、ブラウザ上の操作を確認するもの。製品のReact Native / Expo、ネイティブ解析経路、DB、KIFライブラリを決定したものではない。モックの保存・棋譜準備スクリプトを、そのまま製品のパーサー・合法手判定・ストレージとして採用しない。
