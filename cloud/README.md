...
**Encountered defect and remedy (staging-only):** the first Free run failed with `driver_rejected` (HTTP 404 on `/session`). `/internal/health` showed the singleton still serving the previous image (`buildId faae69bf…`, git `c6d5b52`) after the deploy's completed rollout — a known Cloudflare Containers behavior where a sleeping DO-addressed instance resumes its original image rather than the new one. Remedy: `wrangler containers delete <app-id>` for the three `meeshogi-analysis-mvp-staging-*` apps, then re-run `deploy-staging.sh`; the recreated apps booted the new image and `/internal/health` then reported `buildId 57faf101…` / git `b5f339e`. No code change was needed. For future image upgrades on this worker, expect to delete the three apps (or otherwise recreate instances) after deploy; a sleeping instance does not pick up a rolled-out image.
