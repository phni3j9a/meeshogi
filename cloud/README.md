...
USI driverはPython 3標準ライブラリだけで動き、engine・weight・optionsのdigest、USI handshake、対応option、`isready`を起動ごとに検証します。1 processを排他実行し、局面ごとに `usinewgame` を送ります。Rust helperからroot合法手を列挙し、`effectiveMultiPv=min(requestedMultiPv, rootLegalMoveCount)` をengineへ設定します。同じ完成depthの異なる合法初手にexact scoreと合法なPVが揃った場合だけcompleteです。lowerbound / upperbound、重複rank、重複初手はcompleteにしません。cp / mate scoreはSFENの手番から先手視点へ一度だけ変換します。契約はv2で、centipawnの共有JSON上限はsafe integer ±1,000,000、固定engine adapterの許容範囲は±35,281です。clampやcp-to-mate変換はしません。
