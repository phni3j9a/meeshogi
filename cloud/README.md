...
Mainのstaging記録では固定SFEN smoke、単一合法手、`middlegame-150` gate、post-fix benchmark、およびW3-fix backendのlive検証（dual-container routing、job lifecycle、throw→DLQ、destroy→recovery、cancel保持、mate proof、kill mode、cron recovery）を確認済みです。Worker `meeshogi-analysis-staging` は `free-v1` 用の `AnalysisContainer` (`standard-2`, 1 vCPU) と `precision-v1` 用の `AnalysisContainerPrecision` (`standard-3`, 2 vCPU) を同時にbindし、各々 `max_instances: 1` です。`JobCoordinator` のD1 leaseが両containerを通じて実解析を一件にfenceします。production resourceはありません。fault fixtureはstaging deploy上で `ANALYSIS_FAULT_FIXTURES_ENABLED=1` としてあり、admin token必須の `/v1/internal/fault/arm` 経由でのみarmできます。
...
Fault fixtureは `ANALYSIS_FAULT_FIXTURES_ENABLED=1` の時だけadmin routeでarm可能です。`destroy`はjobの1 dispatchを止め、`throw`はclaim前に指定回数 (1–4) throwしてQueue→DLQを検証します。driverの`MEESHOGI_TEST_SIGSTOP_ENGINE=1` はsearch後にchildを一度SIGSTOPし、通常のtimeout/reap/restartを通すtest-only hookです。stagingではfault arm endpointを検証用に有効化してあり、試験完了後にflag=0で再deployして外部到達不可へ戻します。SIGSTOPのlive注入は `startAndWaitForPorts` の `envVars` で per-start 付与する実装経路があり、shared imageへ恒久ENVは埋め込みません（現deployでは未設定）。

`0001_init.sql` は既存schema、`0002_async_jobs_hardening.sql` はrowsを保持する追加migrationです。旧jobs/positions/cache内容は移行し、旧cacheは`quarantined=1`で保持しつつ読みません。既存terminal jobは履歴として残し、identityを持たない旧queued/running jobは`pre_identity_migration`理由で`failed`/`partial`へ閉じ、未完了positionsもterminalizeして旧Queue messageが新identityで再開しないようにします。`0002`はローカルSQLiteで既存quota/cache/costとactive jobの保持・terminal化まで確認済みで、staging D1にもremote適用済みです（apply時点でactive jobなし、principals・terminal jobs・26件の旧cache rowを保持して移行）。`v3` Durable Object migrationは `AnalysisContainerPrecision` を追加し、既存 `v1`/`v2` は残します。principalはMainがtoken SHA-256 digestで別途seedします。
