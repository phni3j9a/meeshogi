...
describe('async job policy helpers', () => {
  ...
  it('hash components include profile CPU/options and all result-affecting revisions', () => {
    const free = executionIdentityComponents(ANALYSIS_PROFILES['free-v1'], 'engine', 'model', 'sha256:abc');
    const precision = executionIdentityComponents(ANALYSIS_PROFILES['precision-v1'], 'engine', 'model', 'sha256:abc');
    expect(free.instance).toEqual({ type: 'standard-2', vcpu: 1 });
    expect(precision.instance).toEqual({ type: 'standard-3', vcpu: 2 });
    expect(free.revisions.parser).toBe('last-complete-multipv-publication-v1');
    ...
    expect(JSON.stringify(free)).not.toBe(JSON.stringify(executionIdentityComponents(ANALYSIS_PROFILES['free-v1'], 'engine', 'model', 'sha256:other')));
  });
  ...
  it('classifies successful, incomplete, terminal and failed engine states', () => {
    ...
    expect(classifyTerminal('incomplete')).toMatchObject({ evaluationSuccess: false, cacheEligible: false, failed: true, evaluationMissing: true });
    expect(classifyTerminal('resign')).toMatchObject({ processed: true, cacheEligible: false, evaluationMissing: true });
    expect(classifyTerminal('none')).toMatchObject({ processed: true, cacheEligible: false });
  ...
  it('keys cache entries by the immutable identity hash and contract namespace', () => {
    const identity = {
      ...
      modelId: 'model-a', instanceType: 'standard-2' as const, vcpu: 1 as const, executionIdentityHash: 'identity-a',
      executionIdentityComponents: executionIdentityComponents(ANALYSIS_PROFILES['free-v1'], 'engine-a', 'model-a', 'sha256:a'),
    };
