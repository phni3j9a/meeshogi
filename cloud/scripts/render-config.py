...
def main() -> int:
    args = sys.argv[1:]
    if len(args) < 4:
        print("usage: render-config.py TEMPLATE OUTPUT ACCOUNT_ID IMAGE_DIGEST [--verification-stop-engine-once] [--benchmark --build-id BUILD_ID --run-manifest FILE ...]", file=sys.stderr)
        return 2
    ...
    allowed_switches = {"--verification-stop-engine-once", "--benchmark", "--build-id", "--run-manifest"}
    index = 0
